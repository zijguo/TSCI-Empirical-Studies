...
out.table <- data.frame(matrix(0,18,12))
...
for (inter.val in c(0,0.5,1)) {
  for (vio.index in 1:2) {
    for (n in c(1000,3000,5000)) {
      ...
      out.table[r,7:10] <- 3.92*apply(sd.matrix.rf.all,2,mean)[5:8]
      out.table[r,11:12] <- c(mean(q.comp.all),mean(q.robust.all))
      r <- r + 1
    }
  }
}
...
kbl(out.table,"latex",caption = "", align = "c", digits = 2) %>%
  ...
  add_header_above(c(rep(" ", 3), "RF-Cor"=3, "Violation Space"=4, "Est. vio. for RF"=2)) %>%
