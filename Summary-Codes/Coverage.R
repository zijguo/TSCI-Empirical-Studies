...
for (vio.index in 1:2) {
  for (inter.val in c(0,0.5,1)) {
    for (n in c(1000,3000,5000)) {
      ...
      out.table[r,7] = mean(q.comp.all>=1)
      out.table[r,8] <- apply(Cov.basis.oracle.all,2,mean)
      out.table[r,9:10] <- apply(Cov.basis.robust.all,2,mean)
      out.table[r,11] = apply(Cov.TSLS.all,2,mean)
      out.table[r,12] = apply(Cov.oracle.all,2,mean)[1]
      out.table[r,13:14] = apply(Cov.naive.all,2,mean)
      r <- r + 1
    }
  }
}
...
kbl(out.table,"latex",caption = "", align = "c", digits = 2) %>%
  ...
  add_header_above(c(rep(" ", 3), "TSCI-RF"=3, "Invalidity", "TSCI-ba"=3, "TSLS", "Other RF(oracle)"=3)) %>%
...
#     out.table[r,3] <- apply(Cov.oracle.all,2,mean)[2]
#     out.table[r,4] <- apply(Cov.robust.all,2,mean)[2]
#     out.table[r,5] <- apply(Cov.robust.all,2,mean)[4]
#     
#     
#     out.table[r,6] <- apply(Cov.oracle.all,2,mean)[1]
#     
#     
#     out.table[r,7:10] <- apply(Cov.mat.fix.all,2,mean)
#     
#     
#     
#     r <- r + 1
