# use f as IV in TSLS and CF and EE (valid IV setting)
...
sim_func_f <- function(n, a) {
  ...
  returnList <- c(
    beta_TSLS = beta_TSLS
    , bias_TSLS = beta_TSLS - beta
    , sd_TSLS = sd_TSLS
    , cover_TSLS = cover_TSLS
    , length_TSLS = CI_TSLS[2] - CI_TSLS[1]
    , concen = concen
    , beta_CF = beta_CF
    , bias_CF = beta_CF - beta
    , sd_CF = sd_CF
    , cover_CF = cover_CF
    , length_CF = CI_CF[2] - CI_CF[1]
    , beta_EE = beta_EE
