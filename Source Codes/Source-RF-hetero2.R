### Similar to Source-RF-hetero.R, but accommodate for list inputs of vio.space, each entry of the list is a matrix specifying a violation form(e.g., all the interactions between Z and X)
...
TSCI.RF <- function(Y,D,Z,X,intercept=TRUE,vio.space=NULL,layer=TRUE,split.prop=2/3, Omega = NULL,
  ...
  if (!is.null(vio.space)) {
    stopifnot(class(vio.space)[1] == "matrix" | class(vio.space)[1] == "list")
  }
  ...
  # Selection
  outputs <- TSCI.RF.Selection(Y,D,Cov.aug,A1.ind,weight=weight,Q=Q,rm.ind=rm.ind,intercept=intercept,layer=layer,str.thol=str.thol,alpha=alpha)
  return(outputs)
}
...
###        rm.ind: the indices to remove to identify different violation space, for internal use only
...
TSCI.RF.Selection <- function(Y, D, Cov.aug, A1.ind, weight, Q, rm.ind, intercept, layer, str.thol, alpha) {
  ...
  for (index in 1:Q) {
    if (index==Q) {
      ...
      SigmaSqY[index] = mean(eps.hat[[index]]^2)
      stat.outputs <- TSCI.RF.stat(D.rep,Cov.rep[,-rm.ind[[index]]],weight,n,eps.hat[[index]],delta.hat,str.thol=str.thol)
    }
    # the statistics
