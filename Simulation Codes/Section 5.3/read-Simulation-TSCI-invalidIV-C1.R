...
for (a in c(seq(0,1.2,0.1))) {
    total = NULL
    for (round in seq(25)) {
        filename = paste0("results-inter-littleHidden/results"
                          , "-a", a
                          , "-round", round, ".rds")
...
p_bias = ggplot(data=bias_data, aes(x=a, y=Bias, col=variable)) +
    geom_line(data = bias_data[bias_data$variable=="TSLS",], lwd = 1.8) +
    geom_line(lwd = 1) +
    geom_point(size = 1.8) +
...
p_length = ggplot(data=length_data, aes(x=a, y=Length, col=variable)) +
    labs(col="") +
    geom_line(data = length_data[length_data$variable=="TSLS",], lwd = 1.8) +
    geom_line(lwd=1) +
    geom_point(size=1.8) +
    scale_x_continuous(breaks=seq(0,1.2,0.2)) +
    scale_color_manual(values = c("#F8766D","#619CFF","#00BA38"),
...
p_mse = ggplot(data=mse_data, aes(x=a, y=RMSE, col=variable)) +
    geom_line(data = mse_data[mse_data$variable=="TSLS",], lwd = 1.8) +
    geom_line(lwd=1) +
    geom_point(size=1.8) +
    scale_x_continuous(breaks=seq(0,1.2,0.2)) +
...
p1 = ggarrange(ggarrange(p_mse, p_cover, p_length, ncol=3, legend = "none", common.legend = T), 
          p_qcomp, 
          ncol=2, legend = "none", common.legend = F,
          widths = c(3, 1))
p1
