...
sim_func = function(a) {
    ...
    beta_deno_tsls = sum(resid(lm(D ~ X)) * resid(lm(Z ~ X)))
    beta_nume_tsls = sum(resid(lm(Y ~ X)) * resid(lm(Z ~ X)))
    gamma_tsls = coef(lm(D~Z+X))[2]
    Gamma_tsls = coef(lm(Y~Z+X))[2]
    rA_sq_tsls = sum(resid(lm(Z~X))^2)
    ...
    ### decompose the point estimator
    # correspond to gamma * rA_sq * gamma
    beta_deno_tsci = sum(resid(lm(D.rep~Cov.rep))^2)
    # correspond to gamma * rA_sq * Gamma
    beta_nume_tsci0 = sum(resid(lm(D.rep~Cov.rep)) * resid(lm(Y.rep~Cov.rep)))
    beta_nume_tsci = beta_deno_tsci * beta_tsci
    ...
    returnRecord = list(
        ...
        sd_DMLorg = sd_DMLorg,
        # regsDML
        beta_regsdml = beta_regsdml,
        sd_regsdml = sd_regsdml,
        # TSLS
        beta_tsls = beta_tsls,
        gamma_tsls = gamma_tsls,
        Gamma_tsls = Gamma_tsls,
        rA_sq_tsls = rA_sq_tsls,
        beta_nume_tsls = beta_nume_tsls,
        beta_deno_tsls = beta_deno_tsls,
        sd_tsls = sd_tsls,
        concen_tsls = concen_tsls,
        # TSCI
        beta_tsci = beta_tsci,
        beta_tsci0 = coef_tsci0,
        beta_nume_tsci = beta_nume_tsci,
        beta_nume_tsci0 = beta_nume_tsci0,
        beta_deno_tsci = beta_deno_tsci,
        sd_tsci = sd_tsci,
        ivstr = ivstr
    )
    returnRecord
