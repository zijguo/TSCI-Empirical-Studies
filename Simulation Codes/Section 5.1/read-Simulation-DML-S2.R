...
for (a in seq(0.6, 2, 0.2)) {
    ...
    bias_mat = rbind(bias_mat, c(a,
                                 # mean_w(unlist(total[,"beta_DML"]) - beta0),
                                 mean(unlist(total[,"beta_DMLorg"]) - beta0),
                                 # mean_w(unlist(total[,"beta_regsdml"]) - beta0),
                                 # mean_w(unlist(total[,"beta_tsls"]) - beta0),
                                 mean(unlist(total[,"beta_tsci"]) - beta0)
                                 ))
    ### sd
    sd_mat = rbind(sd_mat, c(a,
                             # sqrt(mean_w((unlist(total[,"beta_DML"]) - mean(unlist(total[,"beta_DML"])))^2)),
                             sqrt(mean((unlist(total[,"beta_DMLorg"]) - mean(unlist(total[,"beta_DMLorg"])))^2)),
                             # sqrt(mean_w((unlist(total[,"beta_regsdml"]) - mean(unlist(total[,"beta_regsdml"])))^2)),
                             # sqrt(mean_w((unlist(total[,"beta_tsls"]) - mean(unlist(total[,"beta_tsls"])))^2)),
    ...
    length_mat = rbind(length_mat, c(a,
                                   # mean_w(CI_DML[,2] - CI_DML[,1]),
                                   mean(Winsorize(CI_DMLorg[,2] - CI_DMLorg[,1])),
                                   # mean_w(CI_regsdml[,2] - CI_regsdml[,1]),
                                   # mean_w(CI_tsls[,2] - CI_tsls[,1]),
                                   mean(CI_tsci[,2] - CI_tsci[,1])
                                   ))
    ### RMSE
    rmse_mat = rbind(rmse_mat, c(a,
                               # sqrt(mean_w((unlist(total[,"beta_DML"]) - beta0)^2)),
                               sqrt(mean((unlist(total[,"beta_DMLorg"]) - beta0)^2)),
                               # sqrt(mean_w((unlist(total[,"beta_regsdml"]) - beta0)^2)),
                               # sqrt(mean_w((unlist(total[,"beta_tsls"]) - beta0)^2)),
                               sqrt(mean((unlist(total[,"beta_tsci"]) - beta0)^2))
                               ))
    ### ivstr
    # ivstr_mat = rbind(ivstr_mat, c(a,
    #                                mean_w(unlist(total[,"concen_DML"])),
    #                                # mean_w(unlist(total[,"concen_tsls"])),
    #                                mean_w(unlist(total[,"ivstr"]))
    #                                ))
}
...
p_rmse = ggplot(data = rmse_data, aes(x = a, y = RMSE, col = variable)) +
    ...
    geom_point(size = 1.8) +
    scale_x_continuous(breaks = seq(0.6,2,0.2)) +
    scale_y_continuous(breaks = c(20.5, 40.5, 60.5)) + 
...
p2 = ggarrange(p_rmse, p_cover, p_length, ncol = 3, common.legend = TRUE, legend = "none")
p2
