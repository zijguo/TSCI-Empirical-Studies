library(ivmodel)
source("Source-RF-hetero2_wtSelfPred.R")
...
# nonliearity degree
a = 0.35  # {0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4}

# simulation --------------------------------------------------------------

sim_func <- function(n, a, vio=0) {
    # generate data -----------------------------------------------------------
    ...
    f_func = function(Z, X, a) {
        # 0.5*Z + a*(Z^3/2-25/12) - 0.3*rowSums(X[,1:10]) # the MSE ratio is not large enough
    ...
    returnValue = list(
        res_tsci = c(coef_tsci, coef_tsci0, ivstr)
        , res_mliv = coef_mliv
    )
    returnValue
}
...
for (i in 1:nsim) {
    print(i)
    result = try(sim_func(n, a))
    if ("try-error" %in% class(result)) {
        errorNum = errorNum + 1
        result = try(sim_func(n, a))
        if ("try-error" %in% class(result)) {
            omitNum = omitNum + 1
            result = "Continuos Error"
        }
    }
    total = rbind(total, c(result, errorNum, omitNum))
}
