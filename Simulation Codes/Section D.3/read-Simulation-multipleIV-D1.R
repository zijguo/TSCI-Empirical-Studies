...
p_rmse = ggplot(data=rmse_data, aes(x=a, y=RMSE, col=variable)) +
    geom_line(data=rmse_data[rmse_data$variable=="TSHT",], lwd=1.8) + 
    geom_line(lwd=1) +
    geom_point(size=1.8) +
...
p_cover = ggplot(data=cover_data, aes(x=a, y=Coverage, col=variable)) +
    geom_line(data=cover_data[cover_data$variable=="TSHT",], lwd=1.8) + 
    geom_line(lwd=1) +
    geom_point(size=1.8) +
    scale_x_continuous(breaks=seq(0.1, 1, 0.2)) +
...
p1 = ggarrange(ggarrange(p_rmse, p_cover, p_length, ncol=3, legend = "none", common.legend = T), 
               p_qcomp, 
