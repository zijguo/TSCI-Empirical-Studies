# simulation of binary treatment
...
for(i in 1:nsim) {
  ...
  if(f.index==1){
    D_prob = logistic(f_1(Z)+X%*%alpha+Z*X%*%inter+Error[,1])
  }
  if(f.index==2){
    D_prob = logistic(f_2(Z)+X%*%alpha+Z*X%*%inter+Error[,1])
  }
  if(f.index==3){
    D_prob = logistic(Z*a+X%*%alpha+Z*X%*%inter+Error[,1])
  }
  
  D = rep(NA,n)
  for (j in 1:n) {
    D[j] = rbinom(1,1,D_prob[j])
  }
  
  # D = ifelse(D_prob>0.5,1,0)
...
# setwd("~/TSCI-nonlinear/resource")
# filename = paste("TSCI-ivtest-Setting",f.index,"-Interaction",inter.val,"-Violation",vio.index,"-p",p,"-n",n,"-round",round,".RData",sep="")
# save.image(filename)
