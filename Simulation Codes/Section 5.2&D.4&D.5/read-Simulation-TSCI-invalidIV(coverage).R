...
for (vio in c(1,2)) {
    for (a in c(0,0.5,1)) {
        for (n in c(1000,3000,5000)) {
            ...
            print(dim(total))
            cover_oracle = cover_comp = cover_robust = cover_tsls = cover_init = cover_plug = cover_full = NULL
            cover_plug_hetero = cover_full_hetero = NULL
            ...
            for (i in 1:nrow(total)) {
                ...
                cover_oracle = c(cover_oracle, CI_oracle[1]<=1 & 1<=CI_oracle[2])
                cover_comp = c(cover_comp, CI_comp[1]<=1 & 1<=CI_comp[2])
                cover_robust = c(cover_robust, CI_robust[1]<=1 & 1<=CI_robust[2])
                cover_tsls = c(cover_tsls, CI_tsls[1]<=1 & 1<=CI_tsls[2])
                cover_init = c(cover_init, CI_init[1]<=1 & 1<=CI_init[2])
                cover_plug_hetero = c(cover_plug_hetero, CI_plug_hetero[1]<=1 & 1<=CI_plug_hetero[2])
                cover_full_hetero = c(cover_full_hetero, CI_full_hetero[1]<=1 & 1<=CI_full_hetero[2])
            }
            record = c(
                vio = vio
                , a = a
                ...
                , mean(cover_tsls)
                , mean(cover_init)
                , mean(cover_plug_hetero)
                , mean(cover_full_hetero)
            )
